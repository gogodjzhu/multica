...
| `LOG_COLOR` | unset | Force ANSI color in logs: `true` or `false`. Unset colors only when stderr is an interactive terminal; set `false` to strip color when a container TTY would otherwise leave escape sequences in collected logs |
